#!/usr/bin/env node
// Full GAIA validation matrix. Set H0X_CLI_GAIA_LEVEL to limit by level.
...
async function main() {
  ...
  const extraEnv = {
    ...
    H0X_CLI_GAIA_SPLIT: "validation",
    ATOMIC_AGENT_GAIA_SPLIT: "validation",
  ...
  const level = readEvalEnv("H0X_CLI_GAIA_LEVEL", "ATOMIC_AGENT_GAIA_LEVEL");
  const limit = readEvalEnv("H0X_CLI_GAIA_LIMIT", "ATOMIC_AGENT_GAIA_LIMIT");
  if (level) {
    extraEnv.H0X_CLI_GAIA_LEVEL = level;
    extraEnv.ATOMIC_AGENT_GAIA_LEVEL = level;
  }
  if (limit) {
