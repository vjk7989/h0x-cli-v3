...
| R13 `api.exa.ai/search` or configured endpoint, Exa | Search with configured key | Query/options, `x-api-key` from configured env key (default `EXA_API_KEY`) | Available but keyed branch not selected by current observed environment | Redirect credential forwarding was remediated in source on 2026-09-01; still disclose search query transfer. `src/tools/os/web-search/providers/exa-provider.ts:46`, `src/tools/os/web-search/transport/search-http.ts` |
...
| R15 `api.search.brave.com/res/v1/web/search`, Brave | Optional search provider | Query URL; `X-Subscription-Token`, default `BRAVE_SEARCH_API_KEY` | Not selected; no observed key | Provider-controlled branding; redirect credential forwarding was remediated in source on 2026-09-01. `src/tools/os/web-search/providers/brave-provider.ts:9`, `src/tools/os/web-search/transport/search-http.ts` |
...
| R17 arbitrary public HTTP(S) destinations and DNS resolver | `os.http.request`, `os.web.fetch`; redirects and retries | URL, supplied headers/body for raw HTTP; request metadata for fetch; DNS hostnames | HTTP enabled, approval mode `never`, hostname allowlist `null`; SSRF checks remain. Historical mock tests reproduced unsafe raw HTTP/search redirect behavior; source remediation on 2026-09-01 applies per-hop allowlist checks, strips cross-origin credential headers, and rejects cross-origin `307`/`308` body forwarding | Continue reviewing broader HTTP policy and live process capture; F01 source fix is documented in the ADR. `src/tools/os/http-request.ts`, `http-request-fetch.ts`, `web-search/transport/search-http.ts`, `web-fetch-ssrf-guard.ts:10` |
...
| R24 `huggingface.co/api/*`, `/.../resolve/...`, redirected asset/CDN hosts | Model discovery/download, including custom model URLs | Repo/revision/file metadata, optional `HF_TOKEN`/`HUGGING_FACE_HUB_TOKEN` | No download invoked; tokens absent in observed audit environment. Historical tests reproduced substring-based token routing; source remediation on 2026-09-01 selects tokens from parsed trusted HTTPS origins and re-evaluates manual redirects per hop | Preserve model licenses and verify live CDN/provider behavior separately before release. `src/local-llm/huggingface-api.ts:9`, `download-file.ts`, `model-installer.ts:52` |
...
| R28 custom log/metric/trace sinks, supplied by embedding host | Runtime events, tracing and shutdown | Logs, counts; traces can include prompt/tool args/results | Default log/metric arrays empty; trace sink writes local NDJSON. F03 source remediation on 2026-09-01 redacts credential-like tool args, summaries, and details before trace retention; external sink behavior was not available to audit | Document host boundary; no default remote exporter found. Redaction reduces diagnostic leakage but is not a consent or transport policy. `src/runtime/bootstrap.ts:619`, `:2978`, `src/tracing/trace/trace-recorder.ts` |
